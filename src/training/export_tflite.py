...
def export_fp32(model: tf.keras.Model) -> None:
    ...
    print(f"Saved float32 TFLite model to: {TFLITE_FP32_PATH}")


def export_dynamic_range(model: tf.keras.Model) -> None:
    """
    Dynamic range quantization:
      - Weights quantized, activations still float32
      - Same input/output dtypes as the fp32 model
    ...
    print(f"Saved dynamic-range TFLite model to: {TFLITE_DR_PATH}")
